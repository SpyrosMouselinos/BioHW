...
def generate_guide_upgma_tree(sequences, sub_mat, gap_penalty=-2):
    """
...
def calculate_sp_score(profile_1_items,
                       profile_2_items,
                       sub_mat=None,
                       match_reward=3,
                       mismatch_penalty=-3,
    ...
    I use the provided score matrix to update the substitution scores or simple mismatch / gap penalties.
    """
    # Chose between a substitution matrix or a fixed set of mismatch gap / affine gap combination.
    if sub_mat is not None:
        sub_mat = substitution_matrices.load(sub_mat)

    total_score = 0
    # Calculate SP score by summing up all pairs between two columns
    if profile_2_items is not None:
    ...
    # If profile_2_items is an empty list (or None) : In case we want to calculate the final cost of an MSA for example
    # Then call the same function for all item combinations #
    else:
        for element1, element2 in list(combinations(profile_1_items, 2)):
            if sub_mat is not None:
                if (element1, element2) in sub_mat:
                    total_score += sub_mat[(element1, element2)]
                elif element1 == '-' and element2 == '-':
...
def profile_align(profile1, profile2, sub_mat=None, match_reward=3, mismatch_penalty=-3, gap_penalty=-2):
    """
    ...
    for j in range(N):
        dp[0, j] = j * gap_penalty * max(profile1.shape()[0], profile2.shape()[0])
    for i in range(M):
        dp[i, 0] = i * gap_penalty * max(profile1.shape()[0], profile2.shape()[0])

    # Fill the DP matrix
    for i in range(1, M):
        for j in range(1, N):
            s_diag = calculate_sp_score(profile1.get_column(j),
                                        profile2.get_column(i),
                                        sub_mat=sub_mat,
...
def backtrack_alignment(dp, profile1, profile2, sub_mat=None, match_reward=3, mismatch_penalty=-3, gap_penalty=-2):
    """
    ...
    while i > 0 and j > 0:
        if dp[i][j] == dp[i - 1][j - 1] + calculate_sp_score(profile1.get_column(j), profile2.get_column(i),
                                                             sub_mat=sub_mat, match_reward=match_reward,
                                                             mismatch_penalty=mismatch_penalty,
                                                             gap_penalty=gap_penalty):
            ...
            alignment2.insert(0, profile2.get_column(i))
            alignment1.insert(0, ['-'] * profile1.shape()[0])
            i -= 1
        else:
            alignment1.insert(0, profile1.get_column(j))
            alignment2.insert(0, ['-'] * profile2.shape()[0])
            j -= 1
...
# For this step, the exersize wanted us to take the UPGMA tree (guide tree as given).
# However, I included a function to generate the guide tree, so that I can learn more and create an end to end example.
...
# Run it with BLOSUM62 and gap penalty -1
final_alignment = progressive_alignment(guide_tree,
                                        sub_mat='BLOSUM62',
                                        match_reward=None,
...
total_score = 0
for i in range((final_alignment.shape()[1])):
    total_score += calculate_sp_score(final_alignment.get_column(i), None,
                                      sub_mat='BLOSUM62',
                                      match_reward=None,
                                      mismatch_penalty=None,
                                      gap_penalty=-1)
print(f"Total Assignment Cost: {total_score}")

print('\n\n\n')
# Run it with fixed match reward 1, mismatch penalty -1 and gap penalty -1
final_alignment = progressive_alignment(guide_tree,
                                        sub_mat=None,
                                        match_reward=1,
                                        mismatch_penalty=-1,
